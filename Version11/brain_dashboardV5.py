...
from AdvancedBrainV11 import AdvancedBrainV11  # ✅ 升级到V11
...
class XiaobaiBrainPet(QWidget):
    def __init__(self):
        ...
        # ================== 🔥 新增：走神头像轮播状态 ==================
        self.current_wandering_index = 0  # 当前走神头像索引
        self.wandering_avatar_list = ["wandering01", "wandering02"]  # 走神头像列表
        # ================================================================
    ...
    def check_auto_sleep(self):
        """仅在未聊天、未睡眠、未走神时，随机触发自动睡眠（备用）"""
        if self.is_chat_expanded or self.is_sleeping or brain.is_mind_wandering:
            return
        if random.randint(1, 10000) <= 5:  # 概率从1%降到0.05%
            print("😴 长时间空闲，自动触发睡眠...")
            self.update_avatar('sleep')
            self.trigger_sleep()

    def check_mind_wandering(self):
        """检查是否应该触发走神，并更新界面显示（支持双头像轮播）"""
        try:
            # 睡眠中 → 不处理走神
            if self.is_sleeping:
                return
            ...
            if brain.is_mind_wandering:
                # ================== 🔥 可调整：轮播速度控制 ==================
                # 只有在特定的时间点才切换头像（例如每4秒切换一次）
                # 因为定时器是2秒触发一次，所以用索引取模来控制
                if not hasattr(self, '_wandering_tick'):
                    self._wandering_tick = 0
                self._wandering_tick += 1
                
                # 每2次定时器触发（即4秒）切换一次头像
                if self._wandering_tick % 2 == 0:
                ...
                self.memory_label.setText(f"💭 走神中 | 疲劳度: {fatigue_pct}%")
            else:
                # 清醒状态
                ...
                if self.current_avatar not in ['awake', 'working', 'sleep', 'error']:
                    ...
                    if hasattr(self, '_wandering_tick'):
                        self._wandering_tick = 0
    ...
    def update_brain_status(self):
        ...
        try:
            ...
            self.memory_label.setText(f"📚 记忆数：{total} 条{kg_text}{fatigue_text}")
        except Exception as e:
    ...
    def send_message(self):
        ...
        brain.fatigue_level = max(0.0, brain.fatigue_level - 0.1)  # 交互时轻微降低疲劳度
    ...
    def on_sleep_finish(self, msg):
        ...
        self.update_avatar('awake')
        self.status_label.setText("🧠 大脑已唤醒")
        self.update_brain_status()  # 刷新记忆数和疲劳度显示
    ...
    def trigger_analyze_local(self):
        ...
        try:
            # 遍历所有专家，生成专门的局部连接热力图
            heatmap_dir = os.path.join(self.project_root, "heatmaps/V10")
            total_rate = 0.0
