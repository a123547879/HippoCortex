...
class PersistentCortexV10:
    ...
    def _extract_generic_triple(self, content: str, expert_name: str) -> dict:
        """
        🔥 修复版：废除所有硬编码！根据专家类型/内容自动生成三元组
        不会再让所有记忆都变成同一个subject/attribute
        """
        import re
        ...
        # ===================== 动态生成，永不硬编码 =====================
         # 只有【身份】专家才固定 subject=我
        if expert_name == "身份":
            subject = "我"
            attribute = "个人状态"
        else:
            # 其他专家：用内容前6个字当主体，属性用专家名，彻底打散
            subject = content[:6].strip()
            attribute = expert_name

        # 2. 属性：直接用专家名称（核心！不同专家=不同属性，彻底隔离冲突）
        attribute = expert_name  
    ...
    def _detect_conflict_memories(self, triple: dict) -> List[int]:
        ...
        for mem_id, mem in self.long_term_memory.items():
            ...
            # ===================== 🔥 新增安全检查：只有 subject 和 attribute 都不为空时才匹配 =====================
            mem_subject = meta.get("subject", "")
            mem_attribute = meta.get("attribute", "")
            
            # 只有当记忆的 subject 和 attribute 都不为空，且和查询匹配时，才加入候选
            if (mem_subject 
                and mem_attribute 
        ...
        if self.kg_enabled and batch_texts:
            for content, expert_name, mem_id, entities in zip(batch_texts, batch_experts, mem_ids, batch_entities):
                if mem_id in self.long_term_memory:
                    # 原错误代码：带了 self.llm
                    # self.kg.add_memory_with_entities(content, expert_name, mem_id, entities, self.llm)
                    # 修复后：仅传4个参数（不含llm）
                    self.kg.add_memory_with_entities(content, expert_name, mem_id, entities)
    ...
    def search_memories(self, query_vec: torch.Tensor, query_sdr: torch.Tensor, 
                ...
                query_text: Optional[str] = None) -> List[Tuple[int, float, str, Dict]]:
        """
        ✅ 纯净版检索：删除知识图谱推理/加成/多跳
        仅保留向量检索 + 专家过滤
        """
        # 1. 向量检索
    ...
    def _compat_old_memories(self):
        """兼容旧版记忆，补全元数据（修复版：不乱填 subject/attribute）"""
        now = datetime.datetime.now().isoformat()
        for mem_id, mem in self.index.memories.items():
            meta = mem['metadata']
            ...
            # ===================== 🔥 核心修复：不要乱填 subject/attribute =====================
            # 只有身份记忆才设置 subject='我' 和 attribute='个人状态'
            # 其他记忆保持为空，或者根据内容重新提取
            if meta.get('expert') == '身份':
                # 只有专家是"身份"的记忆，才设置 subject/attribute
                meta.setdefault('is_obsolete', False)
                meta.setdefault('subject', '我')
                meta.setdefault('attribute', '个人状态')
                meta.setdefault('value', mem['content'])
            else:
                # 其他记忆：subject/attribute 保持为空，或者根据内容重新提取
                meta.setdefault('is_obsolete', False)
                meta.setdefault('subject', '')  # 🔥 关键：不要填 '我'
                meta.setdefault('attribute', '')  # 🔥 关键：不要填 '个人状态'
