...
class DynamicExpert(nn.Module):
    ...
    def delete_memory(self, mem_id):
        ...
        # 注意：不删除sdr_list中的数据，因为会导致索引错位
        # 只删除映射关系，检索时会自动忽略没有mem_id的条目
    ...
    def sleep_consolidate(self, epochs=3):
        ...
        logger.info(f"✅ 专家 [{self.name}] 睡眠巩固完成")
        logger.info(f"   修剪了 {num_weak}/{total} ({num_weak/total:.2%}) 个弱连接")
    ...
    def load_weights(self, path):
        ...
        try:
            ...
            logger.info(f"✅ 专家 [{self.name}] 权重加载完成，共 {len(self.sdr_list)} 条记忆")
        except Exception as e:
            logger.error(f"❌ 专家 [{self.name}] 权重加载失败: {e}，初始化新权重")