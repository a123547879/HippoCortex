...
class HippocampusRouterV4(nn.Module):
    """
    ...
    def _initialize_prototypes_with_embedding(self, embedding_model):
        ...
        for sample in concept_samples:
            try:
                ...
                self.expert_prototypes["概念"] = (
                    (1 - alpha) * self.expert_prototypes["概念"] + alpha * emb
                )
            except:
                pass
        ...
        for sample in space_samples:
            try:
                emb = torch.tensor(embedding_model.embed_query(sample), dtype=torch.float32)
                self.expert_prototypes["空间"] = (
                    (1 - alpha) * self.expert_prototypes["空间"] + alpha * emb
                )
            except:
                pass
        ...
        for sample in abstract_samples:
            try:
                emb = torch.tensor(embedding_model.embed_query(sample), dtype=torch.float32)
                self.expert_prototypes["抽象"] = (
                    (1 - alpha) * self.expert_prototypes["抽象"] + alpha * emb
                )
            except:
                pass
        ...
        for sample in visual_samples:
            try:
                emb = torch.tensor(embedding_model.embed_query(sample), dtype=torch.float32)
                self.expert_prototypes["视觉"] = (
                    (1 - alpha) * self.expert_prototypes["视觉"] + alpha * emb
                )
            except:
                pass
        
        self._prototypes_initialized = True
        logger.info("✅ 专家原型向量初始化完成")
        ...
        with torch.no_grad():
            ...
            if text:
                ...
                if any(keyword in text_lower for keyword in ["图片", "图像", "照片", "视觉", "看"]):
                    return "视觉"
                if any(keyword in text_lower for keyword in ["名字", "叫什么", "我是", "你是", "身份", "称呼", "主人", "人物", "是谁"]):
                    return "概念"
                if any(keyword in text_lower for keyword in ["案件", "事件", "地点", "历史", "什么时候", "哪里", "在哪里"]):
                    return "空间"
                if any(keyword in text_lower for keyword in ["名言", "知识", "是什么", "什么是", "概念", "定义"]):
                    return "抽象"
