...
class DynamicExpert(nn.Module):
    ...
    def save_weights(self, path):
        ...
        try:
            torch.save({
                ...
                'metadata_list': self.metadata_list,  # 完整保存新格式metadata
    ...
    def load_weights(self, path):
        ...
        try:
            ...
            self.metadata_list = data.get('metadata_list', [])  # 完整加载新格式metadata
    ...
    def activate_memories(self, memory_clip_vecs: List[torch.Tensor], steps: int = 2) -> Dict:
        ...
        activated_indices = self._retrieve_activated_indices(activation, topk=5)
        activated_memories = [self.get_memory_by_idx(i) for i in activated_indices if i >= 0]
    ...
    def _retrieve_activated_indices(self, activation: torch.Tensor, topk: int = 5) -> List[int]:
        """根据神经激活态，检索被激活的记忆索引"""
        if self.index.ntotal == 0:
            return []
        scores, indices = self.index.search(activation.numpy(), topk)
