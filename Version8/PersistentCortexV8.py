...
class MemoryIndex:
    ...
    def add_negative_association(self, mem_id1: int, mem_id2: int):
        """建立抑制连接：权重=-1，矛盾记忆互相屏蔽"""
        if mem_id1 not in self.memories or mem_id2 not in self.memories:
            return
        self.association_graph[mem_id1].append((mem_id2, -1.0))
        self.association_graph[mem_id2].append((mem_id1, -1.0))
        logger.info(f"🔴 建立抑制突触：{mem_id1} ↔ {mem_id2} (权重=-1)")
...
class PersistentCortexV8:
    def __init__(self, storage_dir: str, experts, llm, kg_enabled: bool = True):
        """
        初始化持久化皮层记忆系统
    ...
    def _detect_conflict_memories(self, triple: dict) -> List[int]:
        ...
        # 调试日志
        logger.info(f"🔍 冲突检测：主体=[{subject}] | 属性=[{attr}]")
        
        if not subject or not attr:
            logger.info("🔍 无有效主体/属性，跳过冲突检测")
            return []
        ...
        for mem_id, mem in self.index.memories.items():
            meta = mem["metadata"]
            mem_sub = meta.get("subject", "")
            mem_attr = meta.get("attribute", "")
            is_obsolete = meta.get("is_obsolete", False)
            
            # 调试日志：打印每一条记忆的信息
            # logger.debug(f"检查记忆 {mem_id}：sub={mem_sub}, attr={mem_attr}, 过时={is_obsolete}")
            
            # 核心规则：主体相同 + 属性相同 + 未过时 = 冲突
            if mem_sub == subject and mem_attr == attr and not is_obsolete:
                conflict_ids.append(mem_id)
                logger.info(f"🚨 发现冲突记忆 ID={mem_id}：{mem['content']}")
    ...
    def search_memories(self, query_vec: torch.Tensor, query_sdr: torch.Tensor, expert_name: Optional[str] = None, top_k: int = config.top_k, min_similarity: float = config.min_similarity) -> List[Tuple[int, float, str, Dict]]:
        ...
        for mem_id, sim, mem in filtered:
            meta = mem['metadata']
            score = 0.4*sim + 0.2*meta.get('importance',0.5) + 0.2*meta.get('activation',0.5) + 0.2*meta.get('recency',0.5)
            scored.append((mem_id, score, mem['content'], mem['metadata']))
        
        # 专家加分
        if expert_name in self.experts:
            expert = self.experts[expert_name]
            for ret in expert.retrieve_multi_hop(query_sdr, 2, top_k):
                score, _, meta, _, mid = ret
                if mid and not meta.get("is_obsolete"):
