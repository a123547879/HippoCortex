...
class PersistentCortexV13:
    ...
    def search_memories(self, query_vec: torch.Tensor, query_sdr: torch.Tensor, 
        ...
        2. 路由专家得分加权排序
        3. 主体过滤防混淆
        4. 🔥 新增：海马体临时记忆优先检索（仿生机制）
        """
        # 🔥 核心新增：提取海马体临时记忆（未巩固的最新记忆）
        def _get_hippocampus_memories():
            hippocampus_results = []
            # 访问海马体缓冲区
            if hasattr(self, 'hippocampus_router'):
                for mem in self.hippocampus_router.hippocampal_buffer:
                    # 计算查询向量与临时记忆的相似度
                    sim = torch.nn.functional.cosine_similarity(query_vec, mem.clip_vec, dim=-1).item()
                    if sim >= min_similarity:
                        # 临时记忆加权 +20% 优先级（最新记忆优先）
                        weighted_score = sim * 1.2
                        hippocampus_results.append((
                            mem.mem_id, 
                            weighted_score, 
                            mem.content, 
                            mem.metadata
        ...
        # ===================== 新架构：有专家得分 → 全局加权检索 + 海马体记忆 =====================
        if expert_scores is not None:
            logger.info(f"🌍 启用全局语义检索 + 专家加权排序 + 海马体临时记忆")
            # 1. 获取皮层长期记忆
            cortex_results = self.retrieve_global_semantic_weighted(
                query_vec=query_vec,
                expert_scores=expert_scores,
                top_k=top_k,
                min_similarity=min_similarity,
            ...
            # 2. 获取海马体临时记忆
            hippo_results = _get_hippocampus_memories()
            # 3. 合并：临时记忆 + 长期记忆，去重+排序
            all_results = hippo_results + cortex_results
            # 去重（防止记忆重复）
            seen_ids = set()
            unique_results = []
        ...
        # ===================== 兼容旧逻辑：无专家得分 → 原有检索 + 海马体记忆 =====================
        logger.warning(f"⚠️ 未获取专家得分，使用兼容检索模式（含海马体临时记忆）")
        # 🔥 注入：先加入海马体临时记忆
        hippo_results = _get_hippocampus_memories()
        ...
        # 2. 合并海马体 + 皮层记忆
        filtered = hippo_results + filtered
        if not filtered:
            return []
