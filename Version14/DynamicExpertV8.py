...
class DynamicExpert(nn.Module):
    def __init__(self, name, initial_dim=2048, max_dim=8192, active_size=60,
        ...
        # ===================== 🔥 新增：突触修剪与新生核心参数 =====================
        # 神经元激活历史（用于判断高频激活区）
        self.neuron_activation_counts = torch.zeros(self.dim, dtype=torch.int32)
        self.neuron_coactivation_matrix = torch.zeros(self.dim, self.dim, dtype=torch.int32)
        
        # 突触更新历史（用于判断突触重要性）
        self.synapse_update_counts = torch.zeros_like(self.synapse, dtype=torch.int32)
        
        # 修剪与新生配置
        self.pruning_enabled = True          # 是否启用修剪
        self.synaptogenesis_enabled = True   # 是否启用新生
        self.pruning_percentile = 0.10       # 每次修剪最弱的10%突触
        self.new_synapse_rate = 0.05         # 每次新生最多新增5%的突触
        self.new_synapse_initial_weight = 0.1 # 新突触的初始权重
        self.max_synapse_density = 0.15      # 最大突触密度（15%）
    ...
    # ===================== 🔥 核心新增：突触修剪（用进废退之"废退"） =====================
    def synaptic_pruning(self, pruning_percentile=None):
        """
        自动修剪弱突触：
    ...
    # ===================== 🔥 核心新增：突触新生（用进废退之"用进"） =====================
    # def synaptogenesis(self, new_synapse_rate=None):
    #     """
    #     定向生成新突触：
    #     1. 找出高频共激活的神经元对
    #     2. 在它们之间建立新突触（如果还没有的话）
    #     3. 模拟大脑在学习过程中建立新连接的过程
    #     """
    ...
    #     with torch.no_grad():
    #         # 1. 找出高频激活的神经元（前20%）
    #         activation_threshold = torch.quantile(self.neuron_activation_counts.float(), 0.8)
    #         # 🔥 修复：将 Tensor 转换为 Python 列表
    ...
    #         # 2. 在高频神经元中，找出共激活次数多但目前没有突触的对
    #         candidate_pairs = []
    #         max_candidates = 1000  # 限制候选数量，避免计算量过大
            
    #         # 随机采样一些高频神经元对（避免全排列组合爆炸）
    #         sample_size = min(len(high_activation_neurons) * 10, max_candidates)
            
    #         for _ in range(sample_size):
    #             # 🔥 修复：high_activation_neurons 已经是 Python 列表，不需要 .item()
    ...
    #         num_to_create = min(len(candidate_pairs), max(int(current_non_zero * new_synapse_rate), 10), max_possible)
    ...
    #             # 新突触初始权重设为小的正值，让后续STDP来强化或削弱
    #             self.synapse.data[j, i] = self.new_synapse_initial_weight * (1.0 + coactivation / 20.0)
    #             self.synapse_update_counts[j, i] = 1
    #             created_count += 1
    ...
    def synaptogenesis(self, new_synapse_rate=None):
        ...
        with torch.no_grad():
            ...
            # 🔥 修复2：如果还是没有候选对，直接从高频神经元中随机生成一些
            if not candidate_pairs:
                logger.info(f"🧠 [{self.name}] 没有足够的共激活对，随机生成新突触")
                for _ in range(20):  # 强制生成20个新突触
                    i = random.choice(high_activation_neurons)
                    j = random.choice(high_activation_neurons)
