...
class DynamicExpert(nn.Module):
    ...
    def _init_local_bias(self):
        """
        类脑局部连接偏置初始化
        同一专家分区内的神经元：初始权重 + 偏置（连接更紧密）
        跨分区（如果有混合标签）：初始权重 * 衰减（连接更稀疏）
        """
        logger.info(f"🧠 [{self.name}] 专家初始化局部连接偏置...")
        ...
        for i in range(self.dim):
            for j in range(self.dim):
                ...
                # 功能分区局部性判断
                same_partition = (self.partition_tags[i] == self.partition_tags[j])
                if same_partition:
                    # 同一分区：加局部偏置，让连接更紧密
                    self.synapse.data[i][j] += self.local_bias_strength
                else:
                    # 跨分区：乘衰减系数，让连接更稀疏
                    self.synapse.data[i][j] *= self.cross_partition_decay
        ...
        logger.info(f"✅ [{self.name}] 局部连接偏置初始化完成 | 局部连接率: {local_connectivity:.2f}%")
    # ========================================================================

    def forward(self, sdr, steps=2, top_k=60):
        """
        SDR在专家网络中传播激活（核心类脑推理）
        局部连接偏置作为初始权重，会自然加速同一分区内的激活传播
        """
        if sdr.dim() == 1:
            sdr = sdr.unsqueeze(0)
    ...
    def sleep_consolidate(self, epochs=3):
        """睡眠巩固：重放记忆+修剪弱突触（全脑区通用，优化身份专家日志）"""
        if not self.sdr_list:
    ...
    def load_weights(self, path):
        ...
        try:
            ...
            logger.info(f"✅ 专家 [{self.name}] 加载完成 | 记忆数: {len(self.sdr_list)} | 局部偏置: {'启用' if self.local_bias_enabled else '禁用'}")
        except Exception as e:
