...
from AdvancedBrainV12 import AdvancedBrainV12  # ✅ 升级到V12（意图驱动版）
...
def plot_core_region_heatmap(expert, name, brain, save_dir="heatmaps/V12_core"):
    """
    ...
    plt.title(f'[{name}] 核心区细节热力图（前200维）\n局部连接率: {local_rate:.2f}% | 记忆数: {len(neuron_to_memories)}', fontsize=14)
...
def plot_local_connectivity_heatmap(expert, name, brain, save_dir="heatmaps/V12"):
    """
...
class XiaobaiBrainPet(QWidget):
    ...
    def trigger_analyze_local(self):
        ...
        try:
            heatmap_dir = os.path.join(self.project_root, "heatmaps/V12_both")
