...
class Thalamus:
    ...
    # ===================== 核心功能2：记忆巩固协调 =====================
    def coordinate_consolidation(self, epochs: int = 3, priority_entities: Optional[List] = None) -> Tuple[int, int]:
        """协调海马体到皮层的记忆巩固过程（修复参数名：priority_entities）"""
        ...
        if priority_entities:
            priority_mem_ids = {m["id"] for m in priority_entities if "id" in m}
            logger.info(f"🧠 元认知：收到 {len(priority_mem_ids)} 条高优先级实体标记")
        ...
        for mem in short_term_memories:
            ...
            if is_priority:
                ...
                logger.debug(f"🧠 元认知优先实体：{content[:25]}... | 加分后重要性={importance:.2f}")
        ...
        if priority_entities:
            logger.info(f"🧠 元认知优先巩固：{priority_consolidated}/{len(priority_mem_ids)} 条高优先级实体成功固化")
